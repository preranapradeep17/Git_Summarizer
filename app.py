...
# ✅ Use SECRET_KEY from .env (fallback optional)
app.secret_key = os.getenv("SECRET_KEY", "fallback-secret-key")

# ✅ Optional: set Flask config from env
...
def api_status():
    ...
    if sess.status == "done":
        response["data"] = {
            ...
            "issues": sess.issues[:5] if sess.issues else [],  # NEW: First 5 issues preview
            "total_issues": len(sess.issues) if sess.issues else 0,
            "dep_analysis": {
...
def health():
    return jsonify({
        "status": "ok",
        "service": "RepoExplainer"
    })


if __name__ == "__main__":
    app.run(
        debug=os.getenv("FLASK_ENV") == "development",  # ✅ controlled by .env
        host="0.0.0.0",
        port=5001
    )
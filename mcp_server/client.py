...
class MCPClient:
    ...
    def get_issues(self, owner: str, repo: str) -> list:
        """NEW: Fetch open issues via MCP tool."""
        result = self.call_tool("get_issues", {"owner": owner, "repo": repo})
        if not result:
            return []

        # call_tool already decodes JSON when possible; for issues this is
        # usually a Python list. Preserve it directly.
        if isinstance(result, list):
            return result

        # If MCP returned plain text (e.g. API/rate-limit error), avoid
        # crashing and return no issues.

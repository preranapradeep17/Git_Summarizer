...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            name="get_issues",
            description="Fetch open GitHub issues (excluding PRs) for a repository",
            inputSchema={
                "type": "object",
                "properties": {
                    "owner": {"type": "string", "description": "GitHub owner/username"},
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    if name == "fetch_repository":
        ...
        # NEW: Fetch open GitHub issues (excluding PRs)
        owner = arguments["owner"]
        repo = arguments["repo"]
        url = f"https://api.github.com/repos/{owner}/{repo}/issues?state=open&per_page=100"
        r = requests.get(url, headers=HEADERS)
        if r.status_code == 403:
            return [TextContent(type="text", text="GitHub API rate limit or auth error. Set GITHUB_TOKEN.")]
        r.raise_for_status()
        issues = r.json()
        filtered_issues = [
            {
            ...
            for issue in issues
            if issue.get("pull_request") is None  # Exclude PRs
        ]
        return [TextContent(type="text", text=json.dumps(filtered_issues, indent=2))]
